    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Stel een pakket samen voor één klant en verwerk tegelijk de voorraad.</p>
            </div>
            ...
            @php
                $pakketRegels = old('pakket_regels', [['product_id' => '', 'aantal' => 1]]);
                if (count($pakketRegels) === 0) {
                    $pakketRegels = [['product_id' => '', 'aantal' => 1]];
                }
            @endphp

            <form method="POST" action="{{ route('voedselpakket.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        ...
                        <select id="klant_id" name="klant_id" required class="mt-1 w-full rounded-lg border {{ $errors->has('klant_id') ? 'border-red-500 bg-red-50 focus:border-red-500 focus:ring-red-500 dark:bg-red-900/20' : 'border-zinc-300 bg-white focus:border-blue-500 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-800' }} px-3 py-2 text-zinc-900 focus:outline-none focus:ring-1 dark:text-zinc-100">
                            ...
                            @foreach ($klanten as $klant)
                                <option value="{{ $klant->Id }}" {{ (string) old('klant_id') === (string) $klant->Id ? 'selected' : '' }}>{{ $klant->GezinsNaam }} - {{ $klant->Postcode }} {{ $klant->Plaats }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" id="datum_samengesteld" name="datum_samengesteld" value="{{ old('datum_samengesteld', now()->format('Y-m-d')) }}" required class="mt-1 w-full rounded-lg border {{ $errors->has('datum_samengesteld') ? 'border-red-500 bg-red-50 focus:border-red-500 focus:ring-red-500 dark:bg-red-900/20' : 'border-zinc-300 bg-white focus:border-blue-500 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-800' }} px-3 py-2 text-zinc-900 focus:outline-none focus:ring-1 dark:text-zinc-100" />
                    ...
                    <div>
                        ...
                        <input type="date" id="datum_uitgifte" name="datum_uitgifte" value="{{ old('datum_uitgifte') }}" class="mt-1 w-full rounded-lg border {{ $errors->has('datum_uitgifte') ? 'border-red-500 bg-red-50 focus:border-red-500 focus:ring-red-500 dark:bg-red-900/20' : 'border-zinc-300 bg-white focus:border-blue-500 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-800' }} px-3 py-2 text-zinc-900 focus:outline-none focus:ring-1 dark:text-zinc-100" />
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2.5 font-medium text-white transition-colors hover:bg-blue-700 dark:hover:bg-blue-500">Pakket opslaan</button>
                    <a href="{{ route('voedselpakket.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-zinc-300 px-6 py-2.5 font-medium text-zinc-700 transition-colors hover:border-zinc-400 hover:bg-zinc-50 dark:border-zinc-600 dark:text-zinc-200 dark:hover:bg-zinc-800">Annuleren</a>
                </div>
            </form>
        </div>
    </div>
